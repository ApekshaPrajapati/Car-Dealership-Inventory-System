...
describe('Vehicle CRUD', () => {
  it('should allow browsing vehicles without a token (public route)', async () => {
  ...
  it('should reject purchase without a token', async () => {
    ...
    const res = await request(app).post(`/api/vehicles/${create.body._id}/purchase`);
...
describe('Purchase & restock', () => {
  ...
  it('should reject purchase when quantity is zero', async () => {
    const create = await request(app)
      .post('/api/vehicles')
      .set('Authorization', `Bearer ${userToken}`)
      .send({ make: 'Nissan', model: 'Altima', category: 'Sedan', price: 23000, quantity: 0 });
